...
touch $iphost
cat << EOF >> $iphost
127.0.0.1       localhost $HOST_NSDB2
...
sleep 3
echocolor "Config network for HOST_NSDB2 node"
ifaces=/etc/network/interfaces
test -f $ifaces.orig || cp $ifaces $ifaces.orig
rm $ifaces
touch $ifaces
cat << EOF >> $ifaces
#Dat IP cho $HOST_NSDB2 node
